Aquí tienes el código completo de `admin/panel-saiayin-do.php`:
...
$defaults = [
    ...
    $prefix . 'subtitle_text' => 'DOJO CREATIVO',
    ...
    $prefix . 'btn_main_text' => 'SAIAYIN DO',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES[$prefix . 'logo'])) {
        $result = processFileUpload($_FILES[$prefix . 'logo'], 'image', 'saiayin-do', $imagesDir, $videosDir);
        if ($result) {
    ...
    if (saveConfig($configFile, $config)) {
        $mensaje .= "✅ Configuración de SAIAYIN DO guardada.";
        $config = loadConfig($configFile);
        $config = array_merge($defaults, $config);
    }
}
...
<body>
<div class="container">
<h1>📄 Panel de SAIAYIN DO</h1>
<p class="subtitle">Configuración de la página SAIAYIN DO</p>
...
<a href="panel-saiayin-do.php" class="active">📄 SAIAYIN DO</a>
