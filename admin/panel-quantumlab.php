...
$defaults = [
    ...
    $prefix . 'subtitle_text' => 'LABORATORIO CUÁNTICO',
    ...
    $prefix . 'btn_main_text' => 'QUANTUMLAB',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES[$prefix . 'logo'])) {
        $result = processFileUpload($_FILES[$prefix . 'logo'], 'image', 'quantumlab', $imagesDir, $videosDir);
        if ($result) {
    ...
    if (saveConfig($configFile, $config)) {
        $mensaje .= "✅ Configuración de QUANTUMLAB guardada.";
        $config = loadConfig($configFile);
        $config = array_merge($defaults, $config);
    }
}
...
<body>
<div class="container">
<h1>📄 Panel de QUANTUMLAB</h1>
<p class="subtitle">Configuración de la página QUANTUMLAB</p>
...
<a href="panel-quantumlab.php" class="active">📄 QUANTUMLAB</a>
