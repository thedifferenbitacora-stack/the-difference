...
$defaults = [
    ...
    $prefix . 'subtitle_text' => 'DISEÑO Y FILOSOFÍA CREATIVA',
    ...
    $prefix . 'btn_main_text' => 'LE TEMATIK',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES[$prefix . 'logo'])) {
        $result = processFileUpload($_FILES[$prefix . 'logo'], 'image', 'le-tematik', $imagesDir, $videosDir);
        if ($result) {
    ...
    if (saveConfig($configFile, $config)) {
        $mensaje .= "✅ Configuración de LE TEMATIK guardada.";
        $config = loadConfig($configFile);
        $config = array_merge($defaults, $config);
    }
}
...
<body>
<div class="container">
<h1>📄 Panel de LE TEMATIK</h1>
<p class="subtitle">Configuración de la página LE TEMATIK</p>
...
<a href="panel-le-tematik.php" class="active">📄 LE TEMATIK</a>
