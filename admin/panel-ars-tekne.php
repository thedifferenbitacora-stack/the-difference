...
$defaults = [
    ...
    $prefix . 'subtitle_text' => 'ARTE Y TÉCNICA',
    ...
    $prefix . 'btn_main_text' => 'ARS TEKNE',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES[$prefix . 'logo'])) {
        $result = processFileUpload($_FILES[$prefix . 'logo'], 'image', 'ars-tekne', $imagesDir, $videosDir);
        if ($result) {
    ...
    if (saveConfig($configFile, $config)) {
        $mensaje .= "✅ Configuración de ARS TEKNE guardada.";
        $config = loadConfig($configFile);
        $config = array_merge($defaults, $config);
    }
}
...
<body>
<div class="container">
<h1>📄 Panel de ARS TEKNE</h1>
<p class="subtitle">Configuración de la página ARS TEKNE</p>
...
<a href="panel-ars-tekne.php" class="active">📄 ARS TEKNE</a>
