...
$prefix = 'quiron_theatre_';
$pageName = 'QUIRÓN THEATRE';
$mensaje = '';
...
$defaults = [
    $prefix . 'title_text' => 'QUIRÓN THEATRE',
    ...
    $prefix . 'subtitle_text' => 'TEATRO Y PERFORMANCE',
    ...
    $prefix . 'btn_main_text' => 'QUIRÓN THEATRE',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES[$prefix . 'logo'])) {
        $result = processFileUpload($_FILES[$prefix . 'logo'], 'image', 'quiron-theatre', $imagesDir, $videosDir);
        if ($result) {
    ...
    if (saveConfig($configFile, $config)) {
        $mensaje .= "✅ Configuración de QUIRÓN THEATRE guardada.";
        $config = loadConfig($configFile);
        $config = array_merge($defaults, $config);
    }
}
...
<title>Panel QUIRÓN THEATRE</title>
<style>
...
<body>
<div class="container">
<h1>📄 Panel de QUIRÓN THEATRE</h1>
<p class="subtitle">Configuración de la página QUIRÓN THEATRE</p>
...
<a href="panel-quiron-theatre.php" class="active"> QUIRÓN THEATRE</a>
</div>
