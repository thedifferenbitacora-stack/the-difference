...
$defaults = [
    ...
    $prefix . 'subtitle_text' => 'TEXTOS Y CONTENIDO',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES[$prefix . 'logo'])) {
        $result = processFileUpload($_FILES[$prefix . 'logo'], 'image', 'texvn', $imagesDir, $videosDir);
        if ($result) {
    ...
    if (saveConfig($configFile, $config)) {
        $mensaje .= "✅ Configuración de TEXVN guardada.";
        $config = loadConfig($configFile);
        $config = array_merge($defaults, $config);
    }
}
...
<body>
<div class="container">
<h1>📄 Panel de TEXVN</h1>
<p class="subtitle">Configuración de la página TEXVN</p>
...
<a href="panel-texvn.php" class="active">📄 TEXVN</a>
